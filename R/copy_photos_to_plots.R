#' Copy and rename photos by nearest plot locality_id
#'
#' Copies (does not move) JPG/JPEG photos from one or more input folders, reads EXIF GPS,
#' assigns each GPS photo to the nearest plot (`locality_id`) and copies into flattened
#' output subfolders. Photos without GPS are skipped by default, but can still be copied
#' when:
#' * `names = TRUE`: the filename already begins with a `locality_id` (any suffix).
#' * `folders = TRUE`: the parent folder name equals a `locality_id` (files are renamed to that ID).
#'
...
#' @param id_field Field name containing locality IDs.
#' @param threshold_m Distance threshold (meters) for "correct" vs "far".
#' @param far_max_m Upper distance threshold (meters) for "far".
#' @param names Logical; allow copying no-GPS photos whose filename already begins with a locality_id into subfolder "name".
#' @param folders Logical; allow copying no-GPS photos whose parent folder name equals a locality_id into subfolder "folder" (renamed to locality_id pattern).
...
#' @param sub_no_gps Subfolder name for no-GPS (logged; not copied unless `names`/`folders` rules match).
...
copy_photos_to_plots <- function(
  ...
  if (!file.exists(plots_path)) stop(sprintf("Plots file not found: %s", plots_path))
  ...
  if (is_gpkg) {
    ...
    if (is.null(plots_layer) || !nzchar(plots_layer)) {
      if (length(lyr) == 1) {
        ...
        stop(sprintf("GeoPackage has multiple layers (%s). Provide plots_layer.", paste(lyr, collapse = ", ")))
      }
    }
  ...
  if (!id_field %in% names(plots)) stop(sprintf("Field '%s' not found in plots data.", id_field))
  if (is.na(sf::st_crs(plots))) stop("Plots data has no CRS defined. Define CRS before running (st_set_crs).")
  ...
  if (file.exists(write_log_csv)) {
    old <- tryCatch(utils::read.csv(write_log_csv, stringsAsFactors = FALSE), error = function(e) NULL)
  ...
  if (length(missing_roots)) warning("Some img_roots do not exist and will be skipped:\n  ", paste(missing_roots, collapse = "\n  "))
  ...
  jpg_files <- unique(unlist(lapply(img_roots, function(root) {
    files <- list.files(root, pattern = "\\.(jpg|jpeg)$", full.names = TRUE, recursive = TRUE, ignore.case = TRUE)
  ...
  if (length(already_done)) {
    ...
    if (!length(jpg_files)) {
      if (verbose) message("Nothing to do: all found sources already copied (based on existing log).")
      return(invisible(data.frame()))
    }
  }

  # ---- read EXIF for this batch ----
  ex <- exifr::read_exif(jpg_files, tags = c("SourceFile", "GPSLatitude", "GPSLongitude"))
  ...
  # ---- prepare locality id sets ----
  ids_set <- unique(as.character(plots[[id_field]]))
  ids_set <- ids_set[!is.na(ids_set) & nzchar(ids_set)]
  ids_ord <- ids_set[order(nchar(ids_set), decreasing = TRUE)]
  ids_key <- tolower(ids_set)
  id_map_folder <- stats::setNames(ids_set, ids_key)
  ...
  if (length(no_gps_idx)) {
    ...
    if (isTRUE(folders)) {
      ...
      hit <- parent_key %in% ids_key
      if (any(hit)) {
        idx <- no_gps_idx[hit]
        id_hit <- unname(id_map_folder[parent_key[hit]])
        dec$status[idx] <- "no_gps_folder"
        dec$keep_original_name[idx] <- FALSE
        dec$base_name[idx] <- id_hit
    ...
    if (isTRUE(names)) {
      ...
      if (length(still)) {
        ...
        match_id <- vapply(base_still, function(b) match_locality_prefix(b, ids_ord), character(1))
        hit2 <- !is.na(match_id)
        if (any(hit2)) {
          idx <- still[hit2]
          dec$status[idx] <- "no_gps_named"
          dec$keep_original_name[idx] <- TRUE
          dec$base_name[idx] <- match_id[hit2]
  ...
  if (length(gps_idx)) {
    pts <- sf::st_as_sf(
      data.frame(lon = ex$GPSLongitude[gps_idx], lat = ex$GPSLatitude[gps_idx]),
      coords = c("lon", "lat"),
      crs = 4326
    )
    ...
    for (j in seq_along(gps_idx)) {
      ...
      if (length(w) >= 2) {
        ...
        ids <- as.character(plots_utm[[id_field]][w[ord]])
        dec$base_name[row_i] <- paste(ids, collapse = " ")

      } else {
        id <- as.character(plots_utm[[id_field]][nearest_idx[j]])
        dec$base_name[row_i] <- id

        if (min_dist_num[j] > threshold_m) {
          
          if (is.finite(far_max_m) && min_dist_num[j] > far_max_m) {
            dec$status[row_i] <- "gps_over_max_distance"
            dec$dest_dir[row_i] <- NA_character_   # important: signals "do not copy"
          } else {
  ...
  for (i in seq_len(n_all)) {
    ...
    if (is.na(current_dir_idx) || dir_idx[i] != current_dir_idx) {
      ...
      if (verbose) message(sprintf("\n[%d/%d] Processing folder: %s", current_dir_idx, length(dir_levels), dirname(src)))
    }
    ...
    if (isTRUE(dec$keep_original_name[i])) {
      ...
      if (is.na(base_no_ext) || base_no_ext == "") base_no_ext <- "unknown_id"
    }
  ...
  if (file.exists(write_log_csv)) {
    old <- tryCatch(utils::read.csv(write_log_csv, stringsAsFactors = FALSE), error = function(e) NULL)
    if (!is.null(old)) log_df <- rbind(old, log_df)
  }
  ...
  if (verbose) {
    ...
    df <- log_df_run  # THIS RUN ONLY
    
    # Helpers
    ok  <- !is.na(df$copy_ok) & isTRUE(df$copy_ok)
    fail <- !is.na(df$copy_ok) & identical(df$copy_ok, FALSE)
    ...
    gps_too_far  <- sum(df$status == "gps_over_max_distance")  # not copied
    ...
    not_copied_plain_no_gps <- sum(df$status == "no_gps")   # intentionally skipped
    not_copied_already      <- sum(already)                 # skipped because log said done
    failed_copies           <- sum(fail)                    # attempted but file.copy failed
    ...
    message("  By filename starts with locality_id (names=TRUE): ", no_gps_name)
    message("  By parent folder = locality_id (folders=TRUE):    ", no_gps_folder)
    ...
    # Compact breakdown table (nice for debugging)
